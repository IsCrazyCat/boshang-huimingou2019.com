...
//        echo "zixuanqu";die;
    ...
    public function JiaoGe(){
        ...
        $users_parameters=M("users_parameters");
        $paramenters=$users_parameters->where("upId=1")->field("paiduoshaoyuan,xiaohaopaidanbi")->find();
    ...
    public function WantInvest(){
        ...
        $users_parameters=M("users_parameters");
        $paramenters=$users_parameters->where("upId=1")->field("upPDLXDay,upPDLXONorOFF,upGuDingLX,upTypeInvestment,upTZMultiples,upMaxMoney,upReact,upTermOfInvestment,paiduoshaoyuan,xiaohaopaidanbi")->find();
    ...
    public function WantInvestAction(){
        ...
        if($aqm["uZFErrorPwdNum"]>=$rs_system["sUZFErrorPwdLockNum"]){
            ...
            $users_parameters=M("users_parameters");
            $paramenters=$users_parameters->where("upId=1")->field("upPDLXONorOFF,upLXType,upGuDingLX,upPDLXDay,upLixiAllOrDay,upTypeInvestment,upTZMultiples,upMaxMoney,paiduoshaoyuan,xiaohaopaidanbi")->find();
            ...
            if($paramenters["upPDLXONorOFF"]==1){
                ...
                if($lixiAllOrDay==2){
                    ...
                    $dataloglixi["mlInfo"]="提供援助".$touzijiner."元，排队期发放全部利息 F";
    ...
    public function YesUnPayInvestAction(){
        ...
        if($aqm["uZFErrorPwdNum"]>=$rs_system["sUZFErrorPwdLockNum"]){
            ...
            $zfsj=$_POST["zfsj"];
            $users_invest=M("users_invest");
            $zinvest=$users_invest->where("uiZhifuDate='{$zfsj}'")->field("uiSuccess,uiId,uiUid,uiUJiner")->find();
            ...
            if($lixiAllOrDay==2){
                ...
                $dataloglixi["mlJiner"]=$liximl;
                $dataloglixi["mlInfo"]="提供援助".$touzijiner."元，发放全部利息 E";
            ...
            $shopjifenlogo['mlInfo']="下级会员提供援助获得商城积分收益：".$shopjifenlogo['mlJiner']."元";// 财务日志说明
...
// $baifenzhiba=$touzijiner*$rs_parameters['zhituijiangb']*0.01;//8%的直推奖
    ...
    public function WantUnInvest(){
        ...
        //     $rs_invest=$users_invest->where("uiTouziEnd=1 AND uiEnd=0 AND uiunShenqing=0 AND uiUid={$uId}")->order("uiTouziEndDate asc")->find();
        //     $this->assign("rs_invest",$rs_invest);
//获取当前用户 所有的还未申请的所有单子ID（可提金额）  QIU
        $rs_invest=$users_invest->field("uiId")->where("uiTouziEnd=1 AND uiEnd=0 AND uiunShenqing=0 AND uiUid={$uId}")->order("uiTouziEndDate asc")->select();
        $array_id=array();
...
//获取当前用户 所有的还未申请的所有单子ID（可提金额）QIU E-N-D
        ...
        $users_parameters=M("users_parameters");
        $paramenters=$users_parameters->where("upId=1")->field("upTypeInvestment,upTZMultiples,upMaxMoney,upReact,upRepeatInvestment")->find();
