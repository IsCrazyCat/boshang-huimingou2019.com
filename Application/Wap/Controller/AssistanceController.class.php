...
class AssistanceController extends LoginTrueController {
    ...
    public function index(){
        ...
        $u_param=M("users_parameters");
        $youShezhi=$u_param->field("yxqXsts,yxqjxsgx")->where("upId=1")->find();
        $tiaoshu=$youShezhi['yxqXsts'];//显示个数
        $gx_time=$youShezhi['yxqjxsgx']*3600;//几小时一更新
        $users=M("users");
        //上次更新时间
        $gengxintime=$users->field("yxqgxtime,xiacikaishi")->where(array("uId"=>$uId))->find();
        if(empty($gengxintime['yxqgxtime'])){
            $rs_invest=$users_invest->where("uiState=0 AND uiShow=1 AND youXuanQu=2")->order('uiId desc')->limit($tiaoshu)->select();
            $data['yxqgxtime']=time();
            $data['xiacikaishi']=$tiaoshu;
            $users->where(array("uId"=>$uId))->save($data);
            $this->assign("rs_invest",$rs_invest);
        }else{
            $cha_time=time()-$gengxintime['yxqgxtime'];
            if($cha_time>$gx_time){//可以更新
                $rs_invest=$users_invest->where("uiState=0 AND uiShow=1 AND youXuanQu=2")->order('uiId desc')->limit($gengxintime['xiacikaishi'],$tiaoshu)->select();
                if(empty($rs_invest)){
                    $rs_invest=$users_invest->where("uiState=0 AND uiShow=1 AND youXuanQu=2")->order('uiId desc')->limit($tiaoshu)->select();
                    $data['yxqgxtime']=time();
                    $data['xiacikaishi']=$tiaoshu;
                    ...
                    $data['yxqgxtime']=time();
                    $data['xiacikaishi']=$tiaoshu+$gengxintime['xiacikaishi'];
                    $users->where(array("uId"=>$uId))->save($data);
                    $this->assign("rs_invest",$rs_invest);
                }

            }else{
                $ww=$gengxintime['xiacikaishi']-$tiaoshu;
                $rs_invest=$users_invest->where("uiState=0 AND uiShow=1 AND youXuanQu=2")->order('uiId desc')->limit($ww,$tiaoshu)->select();
    ...
    public function SuiJiQu(){
        ...
        $u_param=M("users_parameters");
        $youShezhi=$u_param->field("sjqXsts,sjqjxsgx")->where("upId=1")->find();
        $tiaoshu=$youShezhi['sjqXsts'];//显示个数
        $gx_time=$youShezhi['sjqjxsgx']*3600;//几小时一更新
        $users=M("users");
        //上次更新时间
        $gengxintime=$users->field("sjqgxtime,suixiackaishi")->where(array("uId"=>$uId))->find();
        if(empty($gengxintime['sjqgxtime'])){
            $rs_invest=$users_invest->where("uiState=0 AND uiShow=1 AND youXuanQu=1")->order('uiId desc')->limit($tiaoshu)->select();
            $data['sjqgxtime']=time();
            $data['suixiackaishi']=$tiaoshu;
            $users->where(array("uId"=>$uId))->save($data);
            $this->assign("rs_invest",$rs_invest);
        }else{
            $cha_time=time()-$gengxintime['sjqgxtime'];
            if($cha_time>$gx_time){//可以更新
                $rs_invest=$users_invest->where("uiState=0 AND uiShow=1 AND youXuanQu=1")->order('uiId desc')->limit($gengxintime['suixiackaishi'],$tiaoshu)->select();
                if(empty($rs_invest)){
                    $rs_invest=$users_invest->where("uiState=0 AND uiShow=1 AND youXuanQu=1")->order('uiId desc')->limit($tiaoshu)->select();
                    $data['sjqgxtime']=time();
                    $data['suixiackaishi']=$tiaoshu;
                    ...
                    $data['sjqgxtime']=time();
                    $data['suixiackaishi']=$tiaoshu+$gengxintime['suixiackaishi'];
                    $users->where(array("uId"=>$uId))->save($data);
                    $this->assign("rs_invest",$rs_invest);
                }

            }else{
                $ww=$gengxintime['suixiackaishi']-$tiaoshu;
                    if($ww<0){
                        $ww=0;
                    }
                $rs_invest=$users_invest->where("uiState=0 AND uiShow=1 AND youXuanQu=1")->order('uiId desc')->limit($ww,$tiaoshu)->select();
if(empty($rs_invest)){
    $rs_invest=$users_invest->where("uiState=0 AND uiShow=1 AND youXuanQu=1")->order('uiId desc')->limit($tiaoshu)->select();
    $data['sjqgxtime']=time();
    $data['suixiackaishi']=$tiaoshu;
...
//        echo "zixuanqu";die;
    ...
    public function JiaoGe(){
        $this->LoginTrue();
        /*判断是否在可交割时间内*/
        $para=M("users_parameters");
        $sy=$para->field("jgkaishitime,jgjieshutime")->where("upId=1")->find();
        $jgkaishitime=$sy['jgkaishitime'];//开始时间
        $jgjieshutime=$sy['jgjieshutime'];//结束时间
        $nowtime=time();
        $b=date("His",time());
            if($b<$jgkaishitime || $b>$jgjieshutime){
                $this->error("当前不是可交割时间！");
            }

        /*判断是否在可交割时间内 E-N-D*/
          $users_uninvest=M("users_uninvest");
            $unId=$_GET["uIdjs"];        //提供救援人Id
 ...
 if(!empty($jiaoTiem)){
    $undata["zixuanbiaoji"]=2;
        ...
        /*判断该条目是否已经被交割*/
        $shifou=M("users_invest")->where("uiId={$uiId} ")->find();
        if($shifou['uiState']==1){
            $this->error("该条目已经交割!",'/wap/index');
        }
        /*判断该条目是否已经被交割 end*/
        ...
        if($aqm["uZFErrorPwdNum"]>=$rs_system["sUZFErrorPwdLockNum"]){
            ...
            if($uZfPwd!=$aqm["uZfPwd"]){
               ...
               $uiUid=$data["uiUid"]=$_POST["uiUid"];//这个值是投资会员的ID
                ...
                $users_parameters=M("users_parameters");
                $paramenters=$users_parameters->where("upId=1")->find();
                $lixiAllOrDay=$paramenters["upLixiAllOrDay"];//利息每天发放OR一次发放（1是一次发放，2是每天发放）
                if($paramenters["upTypeInvestment"]==0){
                ...
                if($paramenters["upPDLXONorOFF"]==1){
                    ...
                    $userdata["uPDLixiMax"]=$lixi; //这里是排队利息最大值
                    //这里为系统自动每日发放利息做判断值
                    if($lixiAllOrDay==2){
                        //这里是每天发放
                    ...
                    if($lixiAllOrDay==2){
                        ...
                        $dataloglixi["mlInfo"]="提供援助".$touzijiner."元，排队期发放全部利息 D";
        ...
        if($aqm["uZFErrorPwdNum"]>=$rs_system["sUZFErrorPwdLockNum"]){
            ...
            $data["uiImages"]=$_POST["uiImages"];
                ...
                if($result){
					...
					$aqm=$users->where("uId={$uId}")->setInc('tid');
    ...
    public function YesUnPayInvestAction(){
        ...
        if($aqm["uZFErrorPwdNum"]>=$rs_system["sUZFErrorPwdLockNum"]){
            ...
            if($lixiAllOrDay==2){
                ...
                $dataloglixi["mlJiner"]=$liximl;
                $dataloglixi["mlInfo"]="提供援助".$touzijiner."元，发放全部利息 C";
...
//		$myaaa = date( 'Y-m-d H:i:s', time()-14400 );
//        $users_invest = M("users_uninvest");
//        $rs_invest = $users_invest->where(" uuniUid = {$uId} and  uuniStateDate >=  '".$myaaa."' ")->order("uuniId desc")->find();
//		
//		if ( (int)$rs_invest['uuniId'] > 0 ) {
//			$this->error("您有正在被授助的订单！");
//		}
