...
class IndexController extends LoginTrueController {
    public function Index(){
        $this->LoginTrue();
        $systemName=M("system");
        $rs_systemName=$systemName->field("sVersions,sName,sUrl,sCopyrightName,sMaxOnline,sMinOnline,sWeixin,sErweima")->where("sId=1")->find();
...
//            $this->redirect('Users/SetRegStateHuiYuan', array('meiyongdezhi' => 2), 3, '请先激活当前登陆会员，页面跳转中...');
            session("uUser",null);
            $this->error("请先激活当前会员！！！",U('wap/login/index'));

        }else{
            $users=M("users");
            $rs_shouru=$users->where("uId={$uId}")->field("uJiangjin,uLixi,uNowLixi,uVip,uTouziNum,uPDLixi,paiDanBi,xingji,shangchengjifen,tongbuqianbao,dongtaiqianbao,jiHuoMa")->find();
            $this->assign("rs_shouru",$rs_shouru);
            $zonglixi=round(($rs_shouru["uNowLixi"]+$rs_shouru["uPDLixi"])/100,2);
			
            $this->assign("zonglixi",$zonglixi);//利息余额
            $zongjiangjin=round($rs_shouru["uJiangjin"],2);
            $this->assign("zongjiangjin",$zongjiangjin);  //奖金  团队奖励
            //统计手续费的个数
            $mregcodes=M("mregcodes");
            $jihuomacount=$mregcodes->where("mrcMUid={$uId}")->count();
            $this->assign("jihuomacount",$jihuomacount);
            $this->display();
        }
         // $this->display();
    }
    ...
    public function Welcome_app(){
        ...
        $rs_paramet_qiu=$users_param_qiu->where("upId=1")->find();
        ...
        foreach($rs_daoqi as $val_daoqi){
            ...
            if($nowtime>$endtime){
				...
				if ( $rs_paramet_qiu["upLXType"] == 1 ) {
                	 $lixi_qiu=$rs_paramet_qiu["upGuDingLX"]*$val_daoqi['uiUJiner']*$rs_paramet_qiu['upTermOfInvestment']+$rs_users['uLixi'];
					 //upGuDingLX 基准利率固定值  18 ; 救援金额  uiUJiner ; 投资的期限（天）upTermOfInvestment + 
				}
				//如果是 浮动汇率 那么
				if ( $rs_paramet_qiu["upLXType"] == 0 ) {
					$aaa = (int)$rs_paramet_qiu["jixiaoshinei"] ;
					$jingshou = $rs_paramet_qiu["jingshou"] ;//在几小时(内)完成打款静态收益
					$dakuanjifen = $rs_paramet_qiu["dakuanjifen"] ;//在几小时(内)完成打款商城积分收益
					$bbb = (int)$rs_paramet_qiu["upPaymentPeriod"] ;//匹配后付款期限（小时）
					
					$waijingshou = $rs_paramet_qiu["waijingshou"] ;//在几小时(外)完成打款静态收益
					$waidakuanjifen = $rs_paramet_qiu["waidakuanjifen"] ;//在几小时(外)完成打款商城积分收益
					$uiStateDate = strtotime($val_daoqi["uiStateDate"]);//审核成功时间
					$uiSuccessDate = strtotime($val_daoqi["uiSuccessDate"]);//打款成功的时间
					if ( (int)$aaa > 0 && (int)$bbb > 0 ) {
							//如果是2小时内
							if ( $uiSuccessDate - $uiStateDate <= $aaa * 3600 ) {
								$lixi_qiu= ( $jingshou ) * $val_daoqi['uiUJiner'] * $rs_paramet_qiu['upTermOfInvestment']+$rs_users['uLixi'] ;
							}
							//如果是2至4小时
							if ( ( $uiSuccessDate - $uiStateDate > $aaa * 3600 ) &&  ( $uiSuccessDate - $uiStateDate < $bbb * 3600 ) ){
								$lixi_qiu= ( $waijingshou ) * $val_daoqi['uiUJiner'] * $rs_paramet_qiu['upTermOfInvestment'] +$rs_users['uLixi'] ;
							}
					}
				}
                ...
                $userd_qiu["uLixi"]=$lixi_qiu/100;
                $userd_qiu["uNowLixi"]=$lixi_qiu/100;
				
				
				if ( (int)$uId > 0 ) {
                	$users->where("uId={$uId}")->save($userd_qiu);
				}
                //利息日志
                if($uble_qiu==0){
				...
				//如果是 固定汇率 那么
				if ( $rs_paramet_qiu["upLXType"] == 1 ) {
                	$dataloglixi["mlJiner"] = ( $rs_paramet_qiu["upGuDingLX"] / 100 ) * $val_daoqi['uiUJiner'] * $rs_paramet_qiu['upTermOfInvestment'] ;
					/* 
						uiUJiner               救援金额                                        
						upTermOfInvestment     投资期限（天）
						upGuDingLX             固定的利息
					*/
				}
				//如果是 浮动汇率 那么
				if ( $rs_paramet_qiu["upLXType"] == 0 ) {
					...
					if ( (int)$aaa > 0 && (int)$bbb > 0 ) {
							//如果是2小时内
							if ( $uiSuccessDate - $uiStateDate <= $aaa * 3600 ) {
								$dataloglixi["mlJiner"] = ( $jingshou / 100 ) * $val_daoqi['uiUJiner'] * $rs_paramet_qiu['upTermOfInvestment'] ;
							}
							//如果是2至4小时
							if ( ( $uiSuccessDate - $uiStateDate > $aaa * 3600 ) &&  ( $uiSuccessDate - $uiStateDate < $bbb * 3600 ) ){
								$dataloglixi["mlJiner"] = ( $waijingshou / 100 ) * $val_daoqi['uiUJiner'] * $rs_paramet_qiu['upTermOfInvestment'] ;
							}
					}
				}
				
                $dataloglixi["mlInfo"]="提供援助".$val_daoqi['uiUJiner']."元，发放全部利息 B";//李洪顺 B
        ...
        foreach($rs_daoqi as $val_daoqi){
            ...
            $dataloglixi["mlSuccess"]=1;
				...
				*/
				...
				$dataloglixi["mlInfo"]="提供援助".$val_daoqi['uiUJiner']."元，发放全部利息 A";//李洪顺 A
				$nowtimes=date("YmdHis");
				$dataloglixi["mlRandNumber"]="AI".$nowtimes."-".$dataloglixi["mlUid"];
				$money_log->add($dataloglixi);
            }
    ...
    public function UserSign(){
        ...
        if($parameters["upQiandaoONOFF"]==1){
            ...
            if($result){
                ...
                $money_log->add($datalogjiangjin);
                if($parameters["upQiandaoJiangMin"]==0){
                    $this->success("幸福滴手手、  么么哒！ $qiandaojiang 元","",6);
                }else{
                    $this->success("幸福滴手手、  么么哒！ $qiandaojiang 元","",6);
                }
            }else{
