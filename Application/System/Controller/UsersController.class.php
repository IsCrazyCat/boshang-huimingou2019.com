...
class UsersController extends LoginTrueController {
    ...
    public function UpdateUserAction(){
        ...
        $data["uName"]=trim($_POST["uName"]);
        $data["yinhangName"]=trim($_POST["yinhangName"]);
        $data["yinhangNubmer"]=trim($_POST["yinhangNubmer"]);
        $result=$users->where("uId={$uId}")->save($data);
        if($result){
    ...
    public function givecodeshowinfo(){
        $this->LoginTrue();
        $zuser=$_POST["zuser"];
        $data["jiHuoMa"]=$_POST["mrcHuiPrice"];
        $users=M("users");

        $zi_u=$users->where("uUser='{$zuser}'")->find();
        if($zi_u){
            $da['jiHuoMa']=$zi_u['jiHuoMa']+$data["jiHuoMa"];
            $zi_shou=$users->where("uUser='{$zuser}'")->save($da);
            if($zi_shou){
                    /*手续费转入日志*/
                    $add['mrcMUid']=0;//手续费转出人id
                    $add['mrcUseUid']=$zi_u['uId'];//手续费转入人id
                    $add['mrcUseDate']=date("Y-m-d H:i:s",time());//时间
                    $add['mrcPrice']=$data["jiHuoMa"];//手续费个数
                    $add['mrcMorZ']=8;//类型 8是手续费 9是手续费
                    M("mregcodes")->add($add);
                    /*手续费转入日志  end*/
